...
prompt_id_map = {prompt: f"p{idx:0{width}d}" for idx, prompt in enumerate(unique_prompts)}
...
if args.message_id_col_name not in df.columns:
    ...
    df[args.message_id_col_name] = [f"m{idx:0{width}d}" for idx in range(len(df))]
