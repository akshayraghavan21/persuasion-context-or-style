...
CREATE TABLE {args.database_name}.{args.table_name} (
    ...
    {args.op_group_id_col_name} INT(11),
    {args.op_feat_col_name} VARCHAR(36),
    {args.op_value_col_name} INT(11),
...
data_to_insert = df[[args.op_group_id_col_name,
                     args.op_feat_col_name,
                     args.op_value_col_name,
                     args.op_group_norm_col_name]].values.tolist()
...
## I want to make a connection to mysql and load the dataframe to a new table
## First check if table exists, if yes, truncate it
## Else, create new table with schema and then load the data from dataframe to the table
