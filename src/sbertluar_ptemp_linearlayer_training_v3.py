...
class SBERTLUARClassifier(nn.Module):
    def __init__(
        ...
        if not self.only_luar:
            classifier_input_dim = self.luar_dim + self.sbert_dim
        else:
            classifier_input_dim = self.luar_dim
        
        if self.include_prompt:
            print("Prompt has been included, so previously:",classifier_input_dim)
    ...
    def forward(self, 
        ...
        # 3) Combine
        if not self.only_luar:
            combined = torch.cat([luar_emb, sbert_emb], dim=1) 
        else:
            combined = luar_emb
...
# Evaluation
# ----------------------------------------------------------
def evaluate(model, data_loader, tokenizer_luar, device, luar_max_length):
    model.eval()
    all_labels = []
    all_preds = []
    all_probs = []
    ...
    with torch.no_grad():
        for prompt, message, labels in data_loader:
            ...
            tokenized_luar_message = tokenize_luar(message, tokenizer_luar, max_length=luar_max_length)
            tokenized_luar_message = {k: v.to(device).unsqueeze(1) for k, v in tokenized_luar_message.items()}

            tokenized_luar_prompt = tokenize_luar(prompt, tokenizer_luar, max_length=luar_max_length)
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for prompt, message, labels in train_loader:
            ...
            optimizer.zero_grad()
            # Pre-tokenize LUAR
            tokenized_luar_message = tokenize_luar(message, tokenizer_luar, max_length=args.luar_max_length)
            tokenized_luar_message = {k: v.to(device).unsqueeze(1) for k, v in tokenized_luar_message.items()}

            tokenized_luar_prompt = tokenize_luar(prompt, tokenizer_luar, max_length=args.luar_max_length)
            tokenized_luar_prompt = {k: v.to(device).unsqueeze(1) for k, v in tokenized_luar_prompt.items()}
        ...
        # Validation step
        eval_loss, eval_acc, _ = evaluate(model, eval_loader, tokenizer_luar, device, luar_max_length=args.luar_max_length)
    ...
    # -----------------------------
    test_loss, test_acc, test_roc_auc = evaluate(model, test_loader, tokenizer_luar, device, luar_max_length=args.luar_max_length)
    final_metrics = {
        "test_loss": test_loss,
        "test_acc": test_acc,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--luar_max_length", type=int, help="Maximum sequence length for tokenization of LUAR.")
